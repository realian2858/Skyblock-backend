// server.js (v17 - MATCHED PIPELINE: rebuild missing sigs, WI reliable, LBIN accurate)
//
// Key fixes:
// ✅ If sales.signature is missing, rebuild from sales.item_lore/item_bytes before matching
// ✅ If auctions.signature is missing, rebuild from auctions.item_lore/item_bytes before matching
// ✅ normalizeNameForSignature matches ingest: circle-stars -> ✪, weird digits -> ASCII
// ✅ LIVE_ALIVE_MS widened to avoid “live disappeared” between ingest runs
// ✅ strict filters operate only on signature tokens; stars10 authoritative
...
// widen alive window so LBIN doesn’t vanish between Cloud Run jobs
...
========================= */
const PET_ITEM_LABELS = [/* ... unchanged ... */];
const PETITEM_OPTIONS = listToOptions(PET_ITEM_LABELS);

const DYE_LABELS = [  "Aquamarine Dye","Archfiend Dye","Aurora Dye","Bingo Blue Dye","Black Ice Dye","Bone Dye","Brick Red Dye",
...
function scorePartial({ userEnchantsMap, inputStars10, sig, filters }) {
  ...
  for (const [nameKey, inputLvlRaw] of userEnchantsMap.entries()) {
    ...
    score += add;
    matched.push({
      enchant: { tier: tierLabel, label: displayEnchant(nameKey, saleLvl) },
      add,
    });
  }
...
/* =========================
   Helper: build sig if missing (MATCH ingest)
========================= */
...
app.get("/api/recommend", async (req, res) => {
  try {
    ...
    // IMPORTANT: include tier/lore/bytes so we can rebuild missing signatures -> fixes "missing sales"
    const { rows } = await pool.query(
      `
